...
<head>
    ...
    <title>ScubaConnect - Check In</title>
        ...
        <div class="max-w-screen-2xl mx-auto px-2 sm:px-4 lg:px-6 py-8">
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                <div class="p-4"> <!-- Keep reduced padding -->
                    <h1 class="text-2xl font-bold text-gray-800 mb-6 text-center">Check In</h1>
                    ...
                    <div class="flex flex-wrap gap-2 mb-6">
                        ...
                        <a
                            class="inline-flex items-center px-4 py-2 rounded-md bg-blue-500 text-white font-medium cursor-default">
                            <span
                    ...
                    <div class="overflow-x-auto max-w-full">
                        <div class="mx-auto" style="width: 100%;">
                            <table id="check-in-table" class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <?php
                                    $query = $con->query("SELECT * FROM `transaction` NATURAL JOIN `guest` NATURAL JOIN `room` WHERE `status` = 'Check In'") or die(mysqli_error($con));
                                    while ($fetch = $query->fetch_array()) {
                                        ...
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <a href="checkout_query.php?transaction_id=<?php echo $fetch['transaction_id']; ?>"
                                                    onclick="return confirmationCheckin();"
                                                    class="inline-flex items-center px-3 py-1.5 bg-amber-500 text-white rounded-md hover:bg-amber-600 transition">
                                                    <i class="las la-sign-out-alt mr-1"></i> Check Out
                                                </a>
                                            </td>
                                        </tr>
                                        <?php
                    ...
                    <?php if ($query->num_rows == 0): ?>
                        <div class="text-center py-10">
                            <p class="text-gray-500 text-lg">No checked-in guests found</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
    ...
    <script>
        $(document).ready(function () {
            $('#check-in-table').DataTable({
                ...
                "language": {
                    "search": "Search checked-in guests:",
                    "emptyTable": "No checked-in guests found"
                },
                "columnDefs": [
